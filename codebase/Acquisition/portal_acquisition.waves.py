"""
    ...
    Changes
      ...
      2022-12-19 - Made changes to acquisition to support waves files
...
parser = argparse.ArgumentParser(description="Acquire new waves from server using rsync.")
...
with db.cursor() as cur:
    sql = "SELECT * FROM site_wave s left join portal p on s.portal = p.name WHERE "
    ...
    for row in cur:
        ...
        logging.info("Checking rsync for new files for %s %s", row["network"], row["site"])
        ...
        site_rsync_dir = config["directories"]["waves_site_rsync"]
        if config["directories"]["waves_site_rsync"][-1:] != "/":
            site_rsync_dir = "%s/%s/%s/" % (site_rsync_dir, row["network"], row["site"])
        else:
            site_rsync_dir = "%s%s/%s/" % (site_rsync_dir, row["network"], row["site"])
        ...
        findoutput = "/tmp/portal_acquisition_waves-%s-%s" % (row["network"], row["site"])
        f = open(findoutput, "w+")
        ...
        # add if option for rsync.net
            ...
            find = ["ssh", row["sshkey_host"], "find " + row["src_dir"] + " -maxdepth 1 -mmin -" + laststate + " -type f -iname '" + row["file_pattern"] + "'"]

        # regular ssh
        else:
#           find = ["ssh", row["sshkey_host"], "find " + row["src_dir"] + " -maxdepth 1 -mmin -" + laststate + " -type f -iname '" + row["file_pattern"] + "' -exec basename {} \;"]
            find = ["ssh", row["sshkey_host"], "find " + row["src_dir"] + " -maxdepth 1 -mmin -" + laststate + " -type f -iname '" + row["file_pattern"] + "' -exec basename {} ;"]
        logging.debug(find)
        try:
        ...
        # file_pattern is used to distinguish ideal from measured in case they are
        ...
        if row["sshkey_host"] == "localhost":
            rsync = [
                "rsync",
                "-ctdm",
                "--log-file=log/" + row["network"] + "_" + row["site"] + "_wave.log",
                "--log-file-format='%n'",
                "--out-format='%n'",
        ...
        try:
            ...
            for myfile in completed.stdout.decode("ascii").split("\n"):
                if "wls" in myfile:
                    ...
                    wf = WaveFile(site_rsync_dir + myfile)

                    # Get file modification time and compare with state file last mod time
                    statetime = wf.modificationTime + 60
                    if not wf.modificationTime > row["state"]:
                        continue

                    # if we set t0 or t1 check them with the file modification time
                    if args.t0:
                        if wf.modificationTime < float(args.t0):
                            continue
                    if args.t1:
                        if wf.modificationTime > float(args.t1):
                            continue
                    ...
                    # Once the file has been validated, save the file into another holding directory, while at the same time,
                    # changing the filename to reflect the current filenames. This holding directory will be rsynced with the nodes.
                    # holding: /SIO/SDSC/WVLM_SIO_SDSC_2018_06_14_1800.wls
                    ...
                    if not match:
                        ...
                        newdir = "%s/%s/%s/" % (row["wave_node_rsync_dir"], row["network"], row["site"])
                        if not os.path.isdir(newdir):
                            logging.debug("Creating %s", newdir)
                            os.makedirs(newdir, exist_ok=True)
                        # WVLM_PSLR_2022_10_01_0000.wls
                        logging.info("QC completed, copying file (%s) to %s", myfile, newdir)

                        newfile = "WVLM_%s_%s_%s_%s_%s_%s.wls" % (row["network"], row["site"], match.group(1), match.group(2), match.group(3), match.group(4))
                        try:
                            shutil.copy2(site_rsync_dir + myfile, newdir + newfile)
                        except EnvironmentError:
                            # IF copy fails delete the wave file so it can get rsynced again and change statetime
            ...
            if args.dryrun:
                continue
            # Once we go through all the files, set the state time in the database if statetime is not zero (we found a file)
            if statetime == 0:
                continue
            ...
            # Try and update the main state database.  If that doesn't work, update the local one
            logging.debug("Updating state for site %s to %s", row["site"], str(statetime))
            ...
            try:
                ...
                with db_state.cursor() as state_cur:
                    sql = "UPDATE site_wave set state=%s WHERE network=%s AND site=%s"
                    state_cur.execute(sql, (statetime, row["network"], row["site"]))
