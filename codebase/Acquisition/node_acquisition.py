...
# . .. .. . . . .. . . . .. .. . . . .. . . . .. .. . . . .. . . . .. .. . . . .. . .
# cleanly exit my script:
#
#   - close my databases, and make sure any files that
#     didn't get copied to the database get removed from
#     the holding dir.
#
def scriptExit():
...
parser = argparse.ArgumentParser(description="Acquire new radials from portals using rsync.")
...
with db.cursor() as cur:
    ...
    for row in cur:
        ...
        if not row["node_rsync_dir"].endswith("/"):
            row["node_rsync_dir"] = row["node_rsync_dir"] + "/"

        # create my node holding dir
        node_holding_dir = config["directories"]["node_holding"]
        if config["directories"]["node_holding"][-1:] != "/":
        ...
        if row["sshkey_node"] == "localhost":
            rsync = ["rsync", "-crm", "--log-file=log/" + row["name"] + ".log", "--log-file-format='%n'", "--out-format='%n'", "--ignore-missing-args", row["node_rsync_dir"], node_holding_dir]
            ...
            rsync = [
                ...
                "--out-format='%n'",
                "--ignore-missing-args",
                row["sshkey_node"] + ":" + row["node_rsync_dir"],
                node_holding_dir,
            ]
        try:
...
#               if not "ruv" in mydirfile:
                if "ruv" not in mydirfile:
                    continue

                logging.info("rsync found file: %s", mydirfile)
                mydirfile = mydirfile.replace("'", "")

                rf = RadialFile(node_holding_dir + mydirfile)
                ...
                if rf.insertIntoDB():
                    ...
                    finaldir = "%s%s" % (config["directories"]["node_final_dir"], mydir)
